...
class InventoryRepository implements IInventoryRepository {
  ...
  InventoryRepository({required FirebaseFirestore firestore}) : _firestore = firestore;

@override
 Stream<List<Category>> getCategories() {
  return _firestore.collection(categoriesCollections).snapshots().map((snapshot) {
  ...
  Stream<List<Ingredient>> getIngredients(String categoryId) {
    return _firestore
        ...
        .snapshots()
        .map((snapshot) {
      return snapshot.docs.map((doc) => IngredientSerialization.fromJson(doc.data())).toList();
    });
  }
  ...
  Future<List<Ingredient>> searchIngredients(String query) async {
   ...
   final categoriesSnapshot = await _firestore.collection(categoriesCollections).get();
